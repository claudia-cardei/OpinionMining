How to run - determinare opinii pozitive/negative

Conversie din CSV in ARFF
	java -cp opinion.jar com.java.opinionmining.datasets.transform.ConvertCSVtoARFF <file> <hasOpinion>
	
	** file reprezinta fisierul .csv care trebuie convertit in fisier .arff
	** hasOpinion este o valoare booleana ce reprezinta daca fisierul este fisier de training (true), sau este de test (false, nu contine opinii)

OBS: fisierul .csv trebuie sa fie de forma (fara header):
"<continut>","<opinie>"
...
Daca se doreste verificarea modelului prin cross-validare:
	java -jar opinion.jar --trainingDataset=<fisier_training>.arff --applyFilter=true --load=false --crossValidation=true
...
Daca se doreste verificarea modelului pe un set de test:
	java -jar opinion.jar --trainingDataset=<fisier_training>.arff --applyFilter=true --load=false --crossValidation=false --testDataset=<fisier_test>.arff
...
Daca se doreste verificarea modelului prin cross-validare:
	java -jar opinion.jar --trainingDataset=<fisier_training>_transformed.arff --applyFilter=false --load=true --crossValidation=true
	
	** <fisier_training>_transformed.arff este fisierul obtinut mai sus (aferent trebuie sa existe si <fisier_training>_model)
	
Daca se doreste verificarea modelului pe un set de test:
	java -jar opinion.jar --trainingDataset=<fisier_training>_transformed.arff --applyFilter=true --load=false --crossValidation=false --testDataset=<fisier_test>.arff
...
Exemplu:
1. java -cp opinion.jar com.java.opinionmining.datasets.transform.ConvertCSVtoARFF bcr_1400.csv true
2. java -jar opinion.jar --trainingDataset=bcr_1400.arff --applyFilter=true --load=false --crossValidation=true
...
(e mai rapid intrucat nu se mai creaza modelul, ci se incarca direct din fisierul bcr_1400_model):
	java -jar opinion.jar --trainingDataset=bcr_1400_transformed.arff --applyFilter=false --load=true --crossValidation=true
